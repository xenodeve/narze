...
const loaderUtils_2 = require("./loaderUtils");
...
function successfulTypeScriptInstance(loaderOptions, loader, log, colors, compiler, compilerCompatible, compilerDetailsLogMessage) {
    ...
    if (configParseResult.errors.length > 0 && !loaderOptions.happyPackMode) {
        ...
        errors.forEach(error => (0, loaderUtils_1.addErrorToModule)(module, error));
        return {
...
function getCustomTransformers(loaderOptions, program, getProgram) {
    ...
    if (typeof customerTransformers === 'function') {
        ...
        try {
            ...
            throw new Error(`Failed to load customTransformers from "${
            // eslint-disable-next-line @typescript-eslint/restrict-template-expressions
            loaderOptions.getCustomTransformers}": ${err instanceof Error ? err.message : 'unknown error'}`);
        }
        if (typeof customerTransformers !== 'function') {
            throw new Error(`Custom transformers in "${
            // eslint-disable-next-line @typescript-eslint/restrict-template-expressions
            loaderOptions.getCustomTransformers}" should export a function, got ${typeof customerTransformers}`);
        }
        getCustomTransformers = customerTransformers;
    }
...
function reportTranspileErrors(instance, loader) {
    ...
    if (!instance.loaderOptions.happyPackMode) {
        ...
        [...solutionErrors, ...errors].forEach(error => {
            if (module) {
                (0, loaderUtils_1.addErrorToModule)(module, error);
            }
        });
    }
}
