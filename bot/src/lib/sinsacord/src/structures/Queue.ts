...
export class Queue extends Array<Track | UnresolvedTrack> {
  ...
  public add(
    ...
    // Inject addedAt timestamp for track expiry detection
    const injectAddedAt = (t: Track | UnresolvedTrack) => {
      if (t && (t as any).info && !(t as any).info.addedAt) {
        (t as any).info.addedAt = Date.now();
      }
    };

    if (Array.isArray(track)) {
      track.forEach(injectAddedAt);
    } else {
      injectAddedAt(track);
    }
