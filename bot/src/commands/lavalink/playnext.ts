    ...
    autocomplete: async (client: clientBot, interaction: AutocompleteInteraction) => {
        ...
        try {
            if (!query) {
                ...
                // บันทึกผลลัพธ์ลง cache (เฉพาะถ้าไม่ใช่ error)
                if (choices.length > 0 && !choices[0].value.includes('error')) {
                    setCache(cacheKey, choices);
                    const platform = configjson.lavalink_config.default_search_platform;
                    console.log(`[${chalk.bold.blueBright('CACHE')}] Cached result (${platform}) for: ${query}`);
                }
            }
        } catch (error) {
